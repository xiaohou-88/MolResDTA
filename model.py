...
class SMILESModel(nn.Module):           
    ...
    def forward(self, smiles, x, edge_index):  
        v1 = self.transform(smiles)     
        v1, _  = torch.max(v1, -1)   
        v2 = self.gcn(x, edge_index)
        v2 = self.gat(v2, edge_index)
        v2 = torch.mean(v2, dim=0)
        v2 = v2.unsqueeze(0).expand(v1.shape[0], -1)        
...
class Classifier(nn.Sequential):
    ...
    def __init__(self, smiles_model, fasta_model):
        ...
        smiles_output_dim = smiles_model.char_set_len
        fasta_output_dim = 96
        total_input_dim = smiles_output_dim + fasta_output_dim
    ...
    def forward(self, smiles, fasta):
        smiles, x, edge_index = smiles
        v_smiles = self.smiles_model(smiles, x, edge_index)        
